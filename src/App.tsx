...
// Adherence log tracking day/night intake
...
export default function App() {
  ...
  const [glucose, setGlucose] = useState<GlucoseReading[]>(() => JSON.parse(localStorage.getItem('ht_glucose') || JSON.stringify(INITIAL_GLUCOSE)));
  const [bp, setBp] = useState<BloodPressureReading[]>(() => JSON.parse(localStorage.getItem('ht_bp') || JSON.stringify(INITIAL_BP)));
  const [medications, setMedications] = useState<Medication[]>(() => JSON.parse(localStorage.getItem('ht_medications') || JSON.stringify(INITIAL_MEDICATIONS)));
  const [adherence, setAdherence] = useState<Record<string, { morning: number[]; evening: number[] }>>(() => JSON.parse(localStorage.getItem('ht_adherence') || JSON.stringify(INITIAL_ADHERENCE)));
  const [documents, setDocuments] = useState<UploadedDocument[]>(() => JSON.parse(localStorage.getItem('ht_docs') || JSON.stringify(INITIAL_DOCS)));
  const [chatHistory, setChatHistory] = useState<ChatMessage[]>(() => JSON.parse(localStorage.getItem('ht_chat') || JSON.stringify(INITIAL_CHAT)));
  const [news, setNews] = useState<NewsArticle[]>(() => JSON.parse(localStorage.getItem('ht_news') || JSON.stringify(NEWS_ARTICLES)));
  const [selectedNews, setSelectedNews] = useState<NewsArticle | null>(null);

  // Sync to local storage
  useEffect(() => { localStorage.setItem('ht_glucose', JSON.stringify(glucose)); }, [glucose]);
  useEffect(() => { localStorage.setItem('ht_bp', JSON.stringify(bp)); }, [bp]);
  useEffect(() => { localStorage.setItem('ht_medications', JSON.stringify(medications)); }, [medications]);
  useEffect(() => { localStorage.setItem('ht_adherence', JSON.stringify(adherence)); }, [adherence]);
  useEffect(() => { localStorage.setItem('ht_docs', JSON.stringify(documents)); }, [documents]);
  useEffect(() => { localStorage.setItem('ht_chat', JSON.stringify(chatHistory)); }, [chatHistory]);
  useEffect(() => { localStorage.setItem('ht_news', JSON.stringify(news)); }, [news]);
  ...
  const latestGlucose = glucose[0] || { value: 0, date: '', notes: '', mealContext: '', timeOfDay: '' };
  const latestBP = bp[0] || { systolic: 0, diastolic: 0, pulse: 0, date: '', notes: '' };
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-slate-50">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-slate-900 text-slate-300 border-r border-slate-800 flex-shrink-0">
        <div className="p-6 flex items-center space-x-3 border-b border-slate-800 bg-slate-950">
          <div className="w-10 h-10 bg-indigo-600 rounded-xl flex items-center justify-center text-white shadow-lg font-bold text-xl">
            HT
          </div>
          <div>
